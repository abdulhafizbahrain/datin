...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  curveChart(mdPuskesmasId, "PUSKESMAS_BULANAN", "SELECT * WHERE A = 'PUYUNG'");
  pieChart(
    mdPuskesmasId,
    "MASTER",
    "SELECT * WHERE A = 'PUYUNG' AND B > 0 AND C > 0"
  );
  tabelData(
    mdPuskesmasId,
    "MASTER",
    "SELECT F,G,H WHERE A = 'PUYUNG' AND G > 0 AND H > 0 ",
    [2, 3]
  );
...
function totalSasaran(sData) {
  ...
  sData.forEach((element) => {
    html = html + element.SASARAN_KEGIATAN;
  });
...
function pieChart(linkId, sheet, query) {
  getSheetData(linkId, sheet, query, (sheetData) => {
    ...
    sheetData.forEach((element) => {
      ...
      kategori.push(element.PUSKESMAS);
    });
