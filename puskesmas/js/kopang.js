...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  curveChart(mdPuskesmasId, "PUSKESMAS_BULANAN", "SELECT * WHERE A = 'KOPANG'");
  pieChart(
    mdPuskesmasId,
    "MASTER",
    "SELECT * WHERE A = 'KOPANG' AND B > 0 AND C > 0"
  );
  tabelData(
    mdPuskesmasId,
    "MASTER",
    "SELECT F,G,H WHERE A = 'KOPANG' AND G > 0 AND H > 0 ",
    [2, 3]
  );
