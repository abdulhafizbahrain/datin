...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  curveChart(mdBidangId, "DIKES_BULANAN", "SELECT * ");
  pieChart(mdBidangId, "MASTER", "SELECT * WHERE B > 0 AND C > 0");
  tabelData(
    mdBidangId,
    "MASTER",
    "SELECT A,B,C WHERE B > 0 AND C > 0 ",
    [2, 3]
  );

  getSheetData(mdBidangId, "MASTER", "SELECT * ", (sheetData) => {
...
function pieChart(linkId, sheet, query) {
  getSheetData(linkId, sheet, query, (sheetData) => {
    ...
    sheetData.forEach((element) => {
      ...
      capaian = capaian + element.CAPAIAN;
      kategori.push(element.BIDANG);
    });

    target = Number(((target * 100) / 4).toFixed(2));
    capaian = Number(((capaian * 100) / 4).toFixed(2));
    ...
    (function () {
      buildChart(
        ...
        () => ({
          ...
          series: [
            { data: [[5, 6, target - capaian]] },
            // { data: [[8, 4, (100 - target).toFixed(2)]] },
            { data: [[10, 9, capaian]] },
          ],
          dataLabels: {
            style: {
        ...
        {
          colors: ["#facc15", "#16a34a", "#16a34a"],
          markers: {
            strokeColors: "rgb(255, 255, 255)",
          },
        },
        {
          colors: ["#facc15", "#404040", "#16a34a"],
          markers: {
